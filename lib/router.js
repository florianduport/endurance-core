...
const autoWire = function(model, modelName, accessControl = {}) {
  ...
  // Utilisation de `this` pour faire référence à l'objet router
  this.get('/', checkPermissions, async (req, res) => {
    try {
  ...
  this.get('/:id', checkPermissions, restrictToOwner, getItem, (req, res) => {
    res.json(res.item);
  });

  this.post('/', checkPermissions, async (req, res) => {
  ...
  this.patch('/:id', checkPermissions, restrictToOwner, getItem, async (req, res) => {
  ...
  this.delete('/:id', checkPermissions, restrictToOwner, getItem, async (req, res) => {
    try {
